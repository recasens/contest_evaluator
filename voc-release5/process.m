...
[ds, bs] = imgdetect(im, model, thresh); % detect objects in image im
...
if ~isempty(ds)
  if model.type == model_types.MixStar
    if isfield(model, 'bboxpred')
      ...
      [ds, bs] = bboxpred_get(bboxpred, ds, reduceboxes(model, bs)); % bounding box location regression
    else
      warning('no bounding box predictor found');
    end
  end
  [ds, bs] = clipboxes(im, ds, bs); % clip boxes to image boundary
  I = nms(ds, 0.5); % non-maximal suppression
  ds = ds(I,:);
  bs = bs(I,:);
end
